...
function getHouseChoice(): ChoiceId {
...
function ChoiceButton({
  ...
  return onClick ? (
    <button
      ...
      } cursor-pointer transition-[filter,transform] duration-200 ease-out hover:-translate-y-1 hover:brightness-110 focus-visible:-translate-y-1 focus-visible:brightness-110 focus-visible:outline-3 focus-visible:outline-offset-4 focus-visible:outline-focus-ring`}
      type="button"
      onClick={onClick}
      disabled={disabled}
...
function App() {
  ...
  function playRound(choiceId: ChoiceId) {
    ...
    setScores((currentScores) => {
      ...
      if (roundResult === "lose") {
        const nextScore = Math.max(0, currentScores.score - 1);

        setScoreMotion(nextScore === currentScores.score ? "still" : "down");

        if (nextScore !== currentScores.score) {
          setScoreTick((currentTick) => currentTick + 1);
        }
  ...
  return (
    <main className="relative flex h-dvh min-h-[42rem] flex-col items-center overflow-hidden bg-[radial-gradient(circle_at_top,var(--game-bg-start),var(--game-bg-end)_72%)] px-6 py-8 font-barlow text-[var(--page-text)] transition-colors duration-300 max-[720px]:min-h-[45rem] max-[720px]:pb-28">
      ...
      <section className="grid w-full flex-1 place-items-center pt-10 pb-18 max-[720px]:pt-16">
        {!playerChoice || !houseChoice || !result ? (
          <div
            ...
            style={{
              backgroundImage: `url(${assetPath}bg-triangle.svg)`,
            }}
          >
      ...
      <p className="sr-only" role="status" aria-live="polite">
        {playerChoice && houseChoice && result
          ? `You picked ${choices[playerChoice].label}. The house picked ${choices[houseChoice].label}. ${resultText}.`
          : ""}
      </p>
      ...
      >
        <section className="rules-modal grid w-[min(100%,25rem)] gap-8 rounded-lg bg-panel-bg p-8 text-panel-text max-[480px]:min-h-screen max-[480px]:content-between max-[480px]:rounded-none max-[480px]:px-8 max-[480px]:pt-23 max-[480px]:pb-15">
